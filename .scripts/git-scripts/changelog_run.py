...
subject_string = f"^Signed-off-by: {dev_name} <{dev_email}>$"
first_subject_string = f"^Signed-off-by: {dev_name} <{work_email}>$"
	...
	def writeEmail(self):
		for patch in self.patches:
			if (patch.signed_off > 0):
				patch.lines[patch.signed_off] = f"Signed-off-by: {dev_name} <{work_email}>\n"
